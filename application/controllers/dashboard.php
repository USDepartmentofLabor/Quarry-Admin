...
/**
 * APIv2 AdminUI Pilot Dashboard Controller
 *
...
class Dashboard extends CI_Controller {
	...
	// set acl varaible
	private $acl_conf;
	private $super_title='DOL APIv2 Dashboard';
	private $version = '';
	...
	function account_manager() {
		...
		//set table user_id in table open tag
		$table_setup = array('table_open' => '<table id="dataTables-admin" class="table table-striped table-bordered table-hover">');
		$this->table->set_template($table_setup);
		$this->table->set_heading("First Name", "Last Name", "Username", "Status", "Date");

		// load account view
		$data["main_content"] = "dashboard_view/account_manager";
		$this->load->view("dashboard/template", $data);
	}

	function admin_datatable() {
		$this->datatables->select("user_id, first_name, last_name, username, status, date_created")->unset_column("user_id")->from("admin_user");
